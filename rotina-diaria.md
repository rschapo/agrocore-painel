...
- **Tática de busca para preencher valores (aumentar a taxa de acerto):** em vez de uma busca genérica por estado, faça **uma busca pontual por célula pendente**, no formato `cotação <cultura> saca <cidade> <UF> hoje` (ex.: "cotação soja saca Sorriso MT hoje", "preço milho Rio Verde GO hoje"). Fontes que já retornaram dado físico por cidade com sucesso: **Notícias Agrícolas** (cotações por praça) e **Canal Rural** — priorize essas antes de tentar Agrolink/CEPEA diretamente (que frequentemente bloqueiam acesso). Não é preciso tentar as 33 células (11 UF × 3 culturas) toda vez — 3 a 5 buscas pontuais por execução, priorizando as pendentes há mais tempo, já é um progresso real.

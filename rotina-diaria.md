...
- **Leite: ainda em decisão do usuário entre 2 métricas — não escolha sozinho.** Opção 1 = "Leite ao produtor" (CEPEA, captação, R$/litro, já usado na tabela comparativa — dado mensal, não diário). Opção 2 = "Leite UHT" (varejo/industrializado, R$/litro, é o que está no `seriesMap` hoje). Enquanto não houver decisão registrada aqui, **não altere a série `leiteuht`** nem o rótulo da linha "Leite (produtor)" — deixe como está.

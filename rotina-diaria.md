...
- NÃO altere o bloco <header>/logo; só atualize a data em "Atualizado em ...". Isso inclui o bloco `.agc-header-contact` (ícones de e-mail/WhatsApp/Instagram/LinkedIn ao lado do título), o `<a href="https://www.agrocoregestao.com.br">` que envolve a logo do cabeçalho (adicionado em 22/07/2026, URL corrigida em 27/07/2026 — a logo deve continuar linkando para o site institucional, domínio `agrocoregestao.com.br`, não `agrocore.com.br`) e o `<footer class="agc-footer">` institucional no fim da página (logo, contato, redes, linha legal com CNPJ) — **nunca remova, esvazie ou reformate esses blocos** (adicionados em 14/07/2026 e 22/07/2026); eles não fazem parte do conteúdo de dados do dia.
- **Eixo Y dos gráficos (Chart.js) travado em 2 casas decimais fixas** (desde 22/07/2026): os 3 callbacks de `ticks` em `scales.y` (histórico/tendência, Bolsas, Correção monetária) usam `Number(v).toFixed(2)` para evitar erro de ponto flutuante do JS (ex.: exibir "5.180000000000001" em vez de "5.18"). **Não reverta para `v` puro** nesses callbacks.
- Mantenha estrutura, CSS, abas e Chart.js. ACUMULE pontos novos em seriesMap sem apagar os antigos.
- **Consentimento de cookies + Google Analytics (GA4) — LGPD:** desde 11/08/2026 o GA4 (`G-976SX1EF2G`) **NÃO** é mais carregado automaticamente no `<head>`; ele só dispara após o "Aceitar" do banner de cookies. Preserve intactos, em toda edição: (a) o bloco `<script>` de consentimento no FIM do `<body>` (comentado "Consentimento de cookies + Google Analytics … NÃO REMOVER") e (b) o CSS `.agc-cookie` dentro do `<style>`. **NUNCA** recoloque a tag `gtag.js`/`gtag('config', …)` no `<head>` — isso fura o consentimento — nem remova/esvazie o banner. É o que mede os acessos ao painel, agora em conformidade com a Política de Cookies.
...
- **Texto do cabeçalho deve ser CURTO (desde 26/08/2026 — corrigido após virar uma cópia quase idêntica do "Resumo executivo").** O parágrafo dentro do `<header>` (linha com "📅 Atualizado em...") é só: data + dia da semana + pregão coberto + **2-3 destaques do dia** (os fatos mais relevantes, ex.: 1 de mercado + 1 macro/geopolítico), fechando com "Análise completa no **Resumo executivo** abaixo." **NÃO** repita a narrativa inteira ali — a versão longa e detalhada (Wall Street, Ibovespa, câmbio, petróleo, Selic/Focus, CEPEA, CBOT/ICE, tarifas etc.) vive **só** no bloco `.exec`/"Resumo executivo" da aba Visão Geral. Se o cabeçalho estiver passando de ~3 linhas ou repetindo frases quase idênticas às do Resumo executivo, está errado — encurte.
...
## Fontes
CEPEA (soja, milho, arroz, café, boi, frango, suíno, ovos, leite cru e derivados, açúcar/etanol — via Notícias Agrícolas, ver seção "Cana-de-açúcar & Etanol" —, bezerro); Consecana (ATR); Agrolink (grãos por praça/cidade — referencial); BCB (PTAX/SGS/Focus/Copom); IBGE/SIDRA; Conab; USDA/WASDE; B3 (BGI/CCM); Comex Stat/AgroStat (exportação); GDT (lácteos); MAPA/ABPA/WOAH (sanitário); mercado (Ibovespa, S&P, Nasdaq, Dow, VIX, DXY, Brent/WTI, ouro, USD/EUR/JPY).
...
## DXY (índice dólar) — tabela Câmbio
Fonte validada em 22/07: **Investing.com** (`investing.com/indices/usdollar`), via `WebFetch` direto (mesma cautela do CBOT/ICE: é cotação intradia/tempo real, rotule como tal, não como fechamento oficial). Essa página normalmente traz Último, variação do dia e variação em 12 meses direto; Mês/Ano exigem comparar com um ponto de fechamento anterior — use o próprio `seriesMap.dxy` quando já tiver um ponto próximo o bastante (30 dias/desde 1º/jan), senão busque o fechamento de referência (ex.: "DXY dollar index closing price December 31 2025" para a âncora de Ano) e calcule a variação você mesmo, rotulando a base usada (ex.: "≈+3,1% (vs 31/dez/25)"). DXY **não tem previsão do Focus/BC** (não é câmbio brasileiro) — a coluna "Prev. fim 2026" fica **"—"** (estrutural, não "a confirmar").

## Correção monetária — tabela completa (IGP-M, IPCA, IGP-DI, INCC-DI)
Fonte para "Últimos 3 meses" e "No ano": **BCB/SGS**, séries mensais oficiais — códigos `433` (IPCA), `189` (IGP-M), `190` (IGP-DI), `192` (INCC-DI). URL: `https://api.bcb.gov.br/dados/serie/bcdata.sgs.<codigo>/dados/ultimos/<n>?formato=json` (JSON puro, sem necessidade de scraping de HTML — prefira essa API a Grão Direto/Investing.com para este dado). Peça `ultimos/4` para montar "Últimos 3 meses" (últimos 3 valores mensais fechados) e `ultimos/N` cobrindo jan-mês atual para "No ano"; componha (`(1+v1/100)*(1+v2/100)*...-1`), não some os percentuais diretamente. Rotule como "≈" (cálculo próprio) e informe o intervalo de meses usado (ex.: "≈+3,28% (jan-jun)"). **Onde já existir "Acum. 12m (R12)" de publicação oficial direta (ex. IPCA/IGP-M do IBGE/FGV), mantenha essa fonte primária em vez de recalcular** — só calcule R12 você mesmo (mesma composição de 12 valores) para IGP-DI/INCC-DI, que não tinham R12 publicado na tabela. A linha única "INCC / IGP-DI" foi separada em duas linhas (**IGP-DI** e **INCC-DI**) em 22/07/2026 porque são séries diferentes com valores diferentes — não volte a combiná-las numa linha só.

## Câmbio — colunas D-1/Mês/Ano/R12/Prev. fim 2026
- **D-1 e Mês**: para USD/BRL, EUR/BRL, JPY/BRL e DXY, prefira **autocalcular via `seriesMap`** assim que houver pontos suficientes (mesma lógica de tolerância de data da seção de Proteínas acima — ±2 dias pra D-1/semana, ±5 dias pra Mês). Acumule um ponto novo por dia nessas 4 séries no `seriesMap`, mesmo quando o valor não mudar.
- **Ano e R12**: o `seriesMap` do painel não cobre 12 meses nem sempre cobre desde 1º/jan para todo indicador — quando faltar a âncora própria, busque o fechamento histórico externo (BCB/PTAX para USD/BRL — `dadosabertos.bcb.gov.br` ou busca direta por "dólar PTAX fechamento DD/MM/AAAA"; Investing.com para EUR/BRL, JPY/BRL, DXY) e calcule a variação você mesmo, sempre citando a data-base usada.
- **EUR/BRL e JPY/BRL não têm previsão do Boletim Focus/BC** (o Focus só pesquisa USD/BRL) — a coluna "Prev. fim 2026" dessas duas linhas é **"n/d"** (estrutural), não "a confirmar". Só USD/BRL tem essa previsão preenchida.
- Cotações intradia (não fechamento oficial do dia) de EUR/BRL, JPY/BRL e DXY devem usar "≈" no valor e legenda explicando o horário/fonte (mesmo padrão do CBOT/ICE).

## Cana-de-açúcar & Etanol — tabela da aba Commodities
Diagnóstico (15/09/2026): essa tabela ficou **congelada desde julho** (açúcar cristal em "ref. 30/jul", etanol hidratado/anidro em "ref. jul/26", ATR em "mar/26") sem nenhuma marcação de pendência — não havia fonte/tática validada (o CEPEA bloqueia `WebFetch` direto com 403), então a rotina simplesmente não mexia na tabela, e os valores velhos continuaram exibidos como se fossem atuais. Isso fere as regras de datação acima; não repita.

**Fonte validada em 15/09: Notícias Agrícolas** — `https://www.noticiasagricolas.com.br/cotacoes/sucroenergetico`, via `WebFetch` direto (uma única página traz todas as linhas). Cada seção da página tem seu próprio "Atualizado em: DD/MM/AAAA" — **use a data de cada seção, não a data geral da página**. Os indicadores têm cadências diferentes; respeite cada uma:
- **Açúcar cristal (CEPEA)** — seção "Indicador do Açúcar Cristal Cepea/Esalq - SP" (R$/saca de 50 kg + variação diária %). **Cadência diária.** Coluna Data: `ref. DD/mmm`; Observação: variação "no dia". Não confunda com "Açúcar Cristal Empacotado" (R$/5 kg) nem "Refinado Amorfo".
- **Etanol hidratado (CEPEA)** — seção "Indicador Semanal Etanol Hidratado Cepea/Esalq" (R$/litro + variação/semana %). **Não** use a seção "Hidratado Outros Fins". **Cadência semanal.** Coluna Data: `sem. DD–DD/mmm` (ex.: `sem. 07–11/set`); a variação é **"na semana"**, nunca "no dia".
- **Etanol anidro (CEPEA)** — seção "Indicador Semanal Etanol Anidro Cepea/Esalq". Mesmas regras do hidratado (semanal).
- **Reescreva a Observação a cada atualização** com a leitura atual (variação + motivo, se a notícia CEPEA do dia na mesma página explicar — ex.: chuva atrasando moagem). Não deixe comentário antigo (ex.: "safra 25/26 acima da anterior") sobrevivendo ao valor novo.

**Validade / fallback:** o valor está defasado se a data da seção passar de **3 dias úteis** (açúcar) ou de **2 semanas** (etanol). Nesse caso, ou se a página não abrir, tente `cepea.org.br/br/indicador/acucar.aspx` / `cepea.org.br/br/indicador/etanol.aspx` via `WebFetch` (pode dar 403) ou a seção de indicadores da UDOP (`udop.com.br/cotacoes/consecana`). Se nada confirmar: coluna "Último valor" = `a confirmar (desde DD/mmm)`, sem seta/variação, e a Observação guarda o último valor conhecido **com a data dele** (ex.: `último publicado: R$ 2,4622 (sem. 07–11/set)`). **Nunca** deixe um valor antigo na coluna "Último valor" sem essa marcação.

**ATR — Consecana-SP: não há mais fonte pública.** Desde **junho/2026** o Consecana-SP divulga o valor mensal do kg de ATR **exclusivamente a assinantes** (comunicado no site `consecana.com.br`); a Notícias Agrícolas mostra "s/ cotação" para São Paulo e a UDOP só publica AL/SE, PB, PE e PR. Regra:
- Linha "ATR — Consecana-SP": "Último valor" = `n/d`; Data = `restrito desde jun/26`; Observação = `Consecana-SP restrito a assinantes desde jun/26 · ref. pública Consecana-PR: R$ X,XXXX/kg acumulado (mmm/26)`. É `n/d` **estrutural** (não "a confirmar") — fica isento do alerta de 3 dias úteis.
- O valor de referência do PR vem da seção "Açúcar - Preço do ATR" da mesma página da Notícias Agrícolas (coluna **Acumulado**, com o mês de referência) ou de `udop.com.br/cotacoes/consecana` ("Consecana PR — ATR Acumulado"). **Nunca** coloque o valor do PR (ou de outro estado) na coluna de valor da linha SP — ele só aparece na Observação, identificado como PR.
- Se algum dia o valor de SP voltar a aparecer publicamente (com número, não "s/ cotação"), volte a usá-lo normalmente e registre isso no Briefing.

**Legenda da tabela:** substitua a frase "Valores a confirmar são preenchidos pela rotina diária via CEPEA" por uma legenda que diga a fonte e a cadência real: `Fonte: Cepea/Esalq (açúcar diário; etanol semanal) e Consecana (ATR mensal), via Notícias Agrícolas.` Os coeficientes de conversão Consecana podem permanecer.

**Primeira execução com esta regra:** atualize as 4 linhas e a legenda, inclua "Notícias Agrícolas — sucroenergético" na aba Fontes, e registre uma linha no Briefing informando que a tabela Cana & Etanol voltou a ser atualizada (estava parada desde 30/jul) e que o ATR-SP passou a ser restrito a assinantes.
...
| PR | Cascavel | Cascavel | Paranavaí |
| SC | Chapecó | Chapecó | Turvo |
| RS | Passo Fundo | Passo Fundo | Uruguaiana |
...
**Praças de arroz de PR/SC/RS determinadas em 22/07/2026 (regra permanente, revisável, mas não é "a confirmar" de novo).** Escolhidas por serem o polo de produção mais relevante do estado, não a mesma cidade de soja/milho:
- **PR = Paranavaí** (Noroeste do PR — núcleo regional com maior produtividade de arroz irrigado do estado, ex. Santa Isabel do Ivaí). Fonte de cotação: **SEAB/DERAL-PR** (`celepar7.pr.gov.br/sima/cotdiat.asp`), que publica "Arroz Agulhinha em casca tipo 1, sc 60kg" para Paranavaí diariamente — prefira essa fonte oficial estadual a Grão Direto/Notícias Agrícolas para esta UF especificamente.
- **SC = Turvo** (Sul Catarinense — região concentra ~67% da área plantada de arroz do estado; Turvo é o 3º maior produtor).
- **RS = Uruguaiana** (Fronteira Oeste — maior região orizícola do RS e do país).
- Para SC e RS, tente **Notícias Agrícolas** (`noticiasagricolas.com.br/cotacoes/arroz`) primeiro — já lista Uruguaiana e Turvo por nome. **Confira a data de referência da seção de arroz** (ela pode estar bem mais desatualizada que as seções de soja/milho da mesma página — já vimos essa seção datada de meses atrás enquanto o resto da página estava do dia). Se a data for velha (mais de ~5 dias úteis), **não publique o valor** — mantenha "a confirmar" e tente de novo no próximo dia útil; IRGA (`irga.rs.gov.br/cotacoes`) e Epagri/Cepa (`cepa.epagri.sc.gov.br`) são fontes oficiais de contexto/pesquisa, mas na prática publicam só PDFs semanais/mensais defasados, não é um bom lugar para checar cotação do dia.

Regras desta tabela:
- Onde estiver "praça a confirmar" (não deveria mais acontecer para PR/SC/RS após 22/07 — ver acima): pesquise a praça/região mais representativa (Agrolink, CEPEA, IRGA-RS) antes de assumir uma cidade. Se não confirmar, mantenha o rótulo genérico "(ar: praça a confirmar)" em vez de citar uma cidade não verificada.
